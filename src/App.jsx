  ...
  function printReport() {
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>${shopName} — ${periodLabel}</title>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:16px">
        ...
        <div style="text-align:center;background:#ECFDF5;border-radius:8px;padding:8px"><div style="font-size:10px;text-transform:uppercase;color:#6B7280">Profit</div><div style="font-weight:600;color:#059669">${fmt(reportProfit)}</div></div>
        <div style="text-align:center;background:#EFF6FF;border-radius:8px;padding:8px;grid-column:span 2"><div style="font-size:10px;text-transform:uppercase;color:#6B7280">Net Cash</div><div style="font-weight:600;color:#1E3A8A">${fmt(reportSales - reportExp)}</div></div>
      </div>
      ${rowsHtml("Sale", "Sales")}
          ...
          <div className="bg-white rounded-xl max-w-lg w-full shadow-2xl my-8">
            ...
            <div id="monthly-report-print" className="p-6">
              ...
              <div className="grid grid-cols-2 gap-2 mb-4">
                ...
                <div className="text-center bg-[#ECFDF5] rounded-lg py-2">
                  <div className="text-[10px] uppercase text-[#6B7280]">Profit</div>
                  <div className="mono text-sm font-semibold text-[#059669]">{fmt(reportProfit)}</div>
                </div>
                <div className="text-center bg-[#EFF6FF] rounded-lg py-2 col-span-2">
                  <div className="text-[10px] uppercase text-[#6B7280]">Net Cash (Sales − Exp − Restock)</div>
      ...
      {showBackupModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl p-5 w-full max-w-sm">
            <div className="flex items-center justify-between mb-3">
              <h3 className="display font-semibold text-sm">Backup & Restore</h3>
            ...
            <div className="border-t border-[#E4E4F0] pt-3">
              <label
                ...
                className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-semibold border border-[#E4E4F0] hover:bg-[#F5F6FB] cursor-pointer"
              >
                <Upload size={14} /> Restore From Backup
              </label>
              <input id="restore-backup-input" type="file" accept="application/json" onChange={importFullBackup} className="hidden" />
              <p className="text-[11px] text-[#9CA3AF] mt-2">
                Restoring replaces everything currently in the app with what's in the backup file.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center gap-2 shrink-0 flex-wrap">
            ...
            {isAdmin && (
              <button
                ...
                className="flex items-center gap-1.5 text-xs bg-white/10 hover:bg-white/20 px-2.5 py-1.5 rounded-lg"
                title="Backup or restore all data"
              >
                <Download size={12} />
              </button>
            )}
        ...
        {tab === "overview" && (
          <div className="space-y-6">
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
              ...
              <StatCard icon={TrendingUp} label="Profit (This Month)" value={fmt(monthProfit)} accent="green" />
              <StatCard icon={TrendingUp} label="Profit (This Year)" value={fmt(yearProfit)} accent="green" />
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
              ...
              <div className="col-span-2 lg:col-span-1 bg-white rounded-xl border border-[#E4E4F0] p-4">
                <div className="text-[11px] uppercase tracking-wide text-[#6B7280] mb-1">Today's Profit</div>
                <div className="mono text-lg font-semibold text-[#059669]">{fmt(todayProfit)}</div>
              </div>
...
} React, { useState, useEffect, useMemo, useRef } from "react";
import {
  LayoutDashboard, Package, ArrowLeftRight, Plus, Trash2, Pencil, Check,
  Search, Wallet, TrendingUp, TrendingDown, Boxes, AlertTriangle, Store,
  Lock, Download, X, Undo2, ShieldAlert, Printer, FileText, LogOut, Settings, User, Upload, Share2,
